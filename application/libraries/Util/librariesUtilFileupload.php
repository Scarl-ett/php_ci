...
class librariesUtilFileupload
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            */
            ...
            'thumb_nail' => false, // 썸네일 이미지 생성 여부 (php extension gd 라이브러리 필요함) 
    ...
    protected function trim_file_name($file_path, $name, $size, $type, $error, $index, $content_range) {
        ...
        // 파일명 변경
        if (!empty($this->options['change_file_name'])) return $this->options['change_file_name'];
         
        // Remove path information and dots around the filename, to prevent uploading
        // into different directories or replacing hidden system files.
        // Also remove control characters and spaces (\x00..\x20) around the filename:
        
        // 수정 (파일명에 공백 들어가면 잘리는거 때문에)
    ...
    private function convertEncoding($str) {
        ...
        // 파일명 euc-kr로 변경
        $sCharCheck = mb_detect_encoding($str, array("UTF-8", "EUC-KR"));
        if ($sCharCheck == 'UTF-8') {
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file = new stdClass();
        $file->originalName = $this->convertEncoding($name);
        $file->name = $this->convertEncoding($this->get_file_name($uploaded_file, $name, $size, $type, $error,
            $index, $content_range));
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                if ($this->is_valid_image_file($file_path)) {
                    ...
                    if ($this->options['thumb_nail'] === true) {
    ...
    protected function generate_response($content, $print_response = true) {
        if ($print_response) {
            // euc-kr로 json_encode 
            $json = json_encode_euckr($content);
